...
async def validate_member(request):
    try:
        ...
        await cur.execute(
            "select user_id, first_name, last_name from user where student_email = %s "
            "and active = 1 and verified_student_email = 1",
            student_email,
        )
...
def create_session(username):
    ...
    sessions[token] = {"username": username, "expires": time.time() + TTL}
    return token
...
def hash_str(to_hash: str, salt, iterations):
    ...
    return hashlib.pbkdf2_hmac(
        "sha512", to_hash.encode(), salt, iterations, 128
    )
